...
class TaskService(
    ...
    fun createTasks() {
        ...
        val task1 = TaskEntity(targetDate = LocalDateTime.now(), isDone = false, note = "zavolejt mu", createdBy = "david")
        task1.personEntity = personRepository.findById(1L).orElseThrow{ ChangeSetPersister.NotFoundException() }
        taskRepository.save(task1)
        val task2 = TaskEntity(targetDate = LocalDateTime.now(), isDone = true, note = "mozna uz dal vypoved - ozvi se", createdBy = "janicka")
        task2.personEntity = personRepository.findById(2L).orElseThrow{ ChangeSetPersister.NotFoundException() }
        taskRepository.save(task2)
        val task3 = TaskEntity(targetDate = LocalDateTime.now(), isDone = false, note = "udelej si kafe", createdBy = "david")
